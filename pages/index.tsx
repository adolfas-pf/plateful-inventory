...
type SKU = {
  ...
  safety_reserve: number; status: string; funnel: string; hidden: boolean
  avg_weekly_sales: number; expected_restock_date: string | null
}
...
type UERow = {
  sku: string; status: string; product_name: string; hts_code: string
  old_tariff_pct: number; new_tariff_pct: number; manufacturer: string
  manufacturer_country: string; production_lead_time_days: number; cbm: number
  cogs: number; inspection: number; commission: number; tariff_cost: number
  placement_fees: number; shipping_to_us: number; inbound_3pl: number
  storage_3pl: number; cn_landed_cost: number; us_landed_cost: number
  amazon_landed_cogs: number; referral_fees: number; pick_pack_amazon: number
  selling_price_amazon: number; profit_amazon: number; margin_amazon: number
  selling_price_dtc: number; transaction_fees_dtc: number; pick_pack_dtc: number
  shipping_dtc: number; profit_dtc: number; margin_dtc: number
  upc: string; asin: string; current_unit_cost: number; current_unit_cost_to: string
  current_shipping_cost: number; current_shipping_cost_to: string
}
...
type TOChangelogEntry = {
  id: string; to_id: string; field_changed: string; previous_value: string
  new_value: string; comment: string; changed_at: string
}

type CostChangelogEntry = {
  id: string; sku: string; field_changed: string; previous_value: number
  new_value: number; to_number: string; po_number: string; reason: string; changed_at: string
}

type Tab = 'dashboard' | 'backlog' | 'weekly' | 'transfers' | 'ue' | 'import'
...
const skuFunnel = (sku: string) => {
  for (const [f, skus] of Object.entries(FUNNELS)) { if (skus.includes(sku)) return f }
  return 'Other'
}
...
const UE_GROUPS = [
  { label: 'Manufacturing', fields: [
    { key: 'cogs', label: 'COGS' },
    { key: 'inspection', label: 'Inspection' },
    { key: 'commission', label: 'Commission' },
    { key: 'old_tariff_pct', label: 'Old Tariff %' },
    { key: 'new_tariff_pct', label: 'New Tariff %' },
    { key: 'tariff_cost', label: 'Tariff Cost' },
    { key: 'production_lead_time_days', label: 'Lead Time (days)' },
    { key: 'cbm', label: 'CBM' },
  ]},
  { label: 'Shipping & 3PL', fields: [
    { key: 'placement_fees', label: 'Placement/Polybag' },
    { key: 'shipping_to_us', label: 'Shipping to US' },
    { key: 'inbound_3pl', label: 'Inbound 3PL' },
    { key: 'storage_3pl', label: 'Storage 3PL' },
    { key: 'cn_landed_cost', label: 'CN Landed Cost' },
    { key: 'us_landed_cost', label: 'US Landed Cost' },
  ]},
  { label: 'Amazon', fields: [
    { key: 'amazon_landed_cogs', label: 'Amazon Landed COGS' },
    { key: 'referral_fees', label: 'Referral Fees' },
    { key: 'pick_pack_amazon', label: 'Pick & Pack' },
    { key: 'selling_price_amazon', label: 'Selling Price' },
    { key: 'profit_amazon', label: 'Profit' },
    { key: 'margin_amazon', label: 'Margin' },
  ]},
  { label: 'DTC', fields: [
    { key: 'selling_price_dtc', label: 'Selling Price' },
    { key: 'transaction_fees_dtc', label: 'Transaction Fees' },
    { key: 'pick_pack_dtc', label: 'Pick & Pack' },
    { key: 'shipping_dtc', label: 'Shipping' },
    { key: 'profit_dtc', label: 'Profit' },
    { key: 'margin_dtc', label: 'Margin' },
  ]},
  { label: 'Batch Costs', fields: [
    { key: 'current_unit_cost', label: 'Current Unit Cost' },
    { key: 'current_shipping_cost', label: 'Current Shipping Cost' },
  ]},
]
...
export default function Home() {
  ...
  // Collapsed state
  const [collapsedFunnels, setCollapsedFunnels] = useState<Set<string>>(new Set())
  const [collapsedWarehouses, setCollapsedWarehouses] = useState<Set<string>>(new Set())
  const [collapsedTOs, setCollapsedTOs] = useState<Set<string>>(new Set())
  const [collapsedUEFunnels, setCollapsedUEFunnels] = useState<Set<string>>(new Set())
  const [collapsedUEGroups, setCollapsedUEGroups] = useState<Set<string>>(new Set())
  ...
  const [toChangelog, setToChangelog] = useState<TOChangelogEntry[]>([])
  ...
  const [costChangelog, setCostChangelog] = useState<CostChangelogEntry[]>([])
  ...
  async function fetchAll() {
    setLoading(true)
    const [{ data: s }, { data: o }, { data: t }, { data: u }] = await Promise.all([
  ...
  async function fetchTOChangelog(toId: string) {
    const { data } = await supabase.from('to_changelog').select('*').eq('to_id', toId).order('changed_at', { ascending: false })
    if (data) setToChangelog(data)
    setShowTOChangelog(toId)
  }

  async function fetchCostChangelog(sku: string) {
    const { data } = await supabase.from('sku_cost_changelog').select('*').eq('sku', sku).order('changed_at', { ascending: false })
    if (data) setCostChangelog(data)
    setShowCostChangelog(sku)
  }
  ...
  async function saveToEdit() {
    ...
    for (const f of fields) {
      if (toEdits[f] !== undefined && String(toEdits[f]) !== String((editingTO as any)[f] ?? '')) {
        logs.push({ to_id: editingTO.id, field_changed: f, previous_value: String((editingTO as any)[f] ?? ''), new_value: String(toEdits[f]), comment: toComment })
      }
    }
    if (logs.length > 0) {
  ...
  async function toggleHideSku(sku: SKU) {
    await supabase.from('skus').update({ hidden: !sku.hidden }).eq('id', sku.id)
    fetchAll()
  }
  ...
  const skusByFunnel = FUNNEL_ORDER.reduce((acc, f) => {
    acc[f] = visibleSkus.filter(s => FUNNELS[f]?.includes(s.id))
    return acc
  ...
  // Group TOs within each warehouse by TO number
  const groupTOsByNumber = (tos: TransferOrder[]) => {
    const groups: Record<string, TransferOrder[]> = {}
    tos.forEach(t => {
      const key = t.to_number || `__no_to_${t.id}`
  ...
  const weeklyData = (() => {
    ...
    orders.forEach(o => {
      const d = new Date(o.order_date); const mon = new Date(d)
  ...
  const ueByFunnel = FUNNEL_ORDER.reduce((acc, f) => {
    acc[f] = ueData.filter(r => FUNNELS[f]?.includes(r.sku))
    return acc
  }, {} as Record<string, UERow[]>)
  ...
  const th: React.CSSProperties = { padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: '#374151', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap', background: '#f9fafb' }
  const td: React.CSSProperties = { padding: '9px 14px', borderBottom: '1px solid #f3f4f6', fontSize: 13 }
  const funnelHeader = (collapsed: boolean, onClick: () => void, label: string, count: number, extras?: React.ReactNode): React.ReactNode => (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#f3f4f6', borderRadius: collapsed ? 8 : '8px 8px 0 0', cursor: 'pointer', userSelect: 'none', border: '1px solid #e5e7eb', marginBottom: collapsed ? 8 : 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 12, color: '#9ca3af' }}>{count} SKUs</span>
        {extras}
      </div>
      <span style={{ fontSize: 14, color: '#9ca3af', transform: collapsed ? 'rotate(-90deg)' : 'none', transition: 'transform 0.15s' }}>▾</span>
    </div>
  )
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', minHeight: '100vh', background: '#f9fafb', color: '#111' }}>
      ...
      {/* Sticky header */}
      <div style={{ ...sticky, background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      {/* Sticky tabs */}
      <div style={{ ...sticky, top: 49, background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '0 24px', display: 'flex', gap: 20 }}>
        {([['dashboard','Dashboard'],['backlog','Backlog'],['weekly','Weekly'],['transfers','Transfer Orders'],['ue','Unit Economics'],['import','Import']] as [Tab,string][]).map(([t,label]) => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '12px 0', background: 'none', border: 'none', cursor: 'pointer', borderBottom: tab === t ? '2px solid #111' : '2px solid transparent', fontWeight: tab === t ? 600 : 400, fontSize: 14, color: tab === t ? '#111' : '#6b7280', whiteSpace: 'nowrap' }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
        ...
        {!loading && tab === 'dashboard' && (
          <div>
            ...
            {FUNNEL_ORDER.map(funnel => {
              const fskus = skusByFunnel[funnel] || []
              if (fskus.length === 0) return null
              const collapsed = collapsedFunnels.has(funnel)
              return (
                <div key={funnel} style={{ marginBottom: 16 }}>
                  {funnelHeader(collapsed, () => toggle(collapsedFunnels, funnel, setCollapsedFunnels), funnel, fskus.length) as any}
                  {!collapsed && (
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <th style={th}>Reserve</th>
                            <th style={th}>Avg/Week</th>
                          ...
                          {fskus.map(s => {
                            ...
                            return (
                              <tr key={s.id} style={{ opacity: s.hidden ? 0.5 : 1 }}>
                                <td style={td}><span style={{ background: '#f3f4f6', padding: '2px 7px', borderRadius: 4, fontSize: 11, fontFamily: 'monospace' }}>{s.id}</span></td>
                                ...
                                <td style={td}><span style={{ fontWeight: 600, color: daysColor(days) }}>{daysLabel(days)}</span></td>
                                <td style={td}>
                                  <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 12, background: isOut ? '#fee2e2' : isCrit ? '#fef3c7' : '#dcfce7', color: isOut ? '#dc2626' : isCrit ? '#d97706' : '#16a34a' }}>
                                    {isOut ? 'Out' : isCrit ? 'Critical' : 'OK'}
                                  </span>
                                </td>
        ...
        {!loading && tab === 'backlog' && (
          <div>
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead><tr style={{ borderBottom: '1px solid #e5e7eb' }}>{['Order','Customer','SKU','Qty','Date','Status','Actions'].map(h => <th key={h} style={th}>{h}</th>)}</tr></thead>
                <tbody>
                  ...
                  : filteredOrders.map(o => (
                    <tr key={o.id}>
                      ...
                      <td style={td}><span style={{ background: '#f3f4f6', padding: '2px 7px', borderRadius: 4, fontSize: 11 }}>{o.sku_id}</span></td>
        ...
        {!loading && tab === 'weekly' && (
          <div>
            ...
            {weeklyData.length === 0 ? <div style={{ color: '#9ca3af', padding: 40, textAlign: 'center' }}>No order data yet — import a Shopify CSV first.</div> : (
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead><tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                    ...
                    {skuIds.map(s => <th key={s} style={th}>{s}</th>)}
                  </tr></thead>
                  <tbody>
        ...
        {!loading && tab === 'transfers' && (
          <div>
            ...
            {Object.entries(groupedTransfers).sort(([a],[b]) => a.localeCompare(b)).map(([dest, tos]) => {
              const destCollapsed = collapsedWarehouses.has(dest)
              ...
              return (
                <div key={dest} style={{ marginBottom: 20 }}>
                  <div onClick={() => toggle(collapsedWarehouses, dest, setCollapsedWarehouses)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#1f2937', borderRadius: destCollapsed ? 8 : '8px 8px 0 0', cursor: 'pointer', userSelect: 'none' }}>
                  ...
                  {!destCollapsed && (
                    <div style={{ border: '1px solid #e5e7eb', borderTop: 'none', borderRadius: '0 0 8px 8px', background: '#fff', overflow: 'hidden' }}>
                      {Object.entries(byTO).sort(([a],[b]) => a.localeCompare(b)).map(([toNum, toRows]) => {
                        ...
                        const toCollapsed = collapsedTOs.has(toKey)
                        ...
                        return (
                          <div key={toKey} style={{ borderBottom: '1px solid #f3f4f6' }}>
                            <div onClick={() => toggle(collapsedTOs, toKey, setCollapsedTOs)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', background: '#f9fafb', cursor: 'pointer', userSelect: 'none' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            {!toCollapsed && (
                              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                                <thead><tr style={{ background: '#fff', borderBottom: '1px solid #f3f4f6' }}>
                                  {['SKU','Qty','Pick Up','ETA','Method','Status',''].map(h => <th key={h} style={{ ...th, background: '#fff', fontSize: 11 }}>{h}</th>)}
                                </tr></thead>
                                <tbody>
                                  {toRows.sort((a,b) => (a.eta_destination||'').localeCompare(b.eta_destination||'')).map(t => (
                                    <tr key={t.id}>
                                      <td style={td}><span style={{ background: '#f3f4f6', padding: '2px 7px', borderRadius: 4, fontSize: 11, fontFamily: 'monospace' }}>{t.sku}</span></td>
        ...
        {!loading && tab === 'ue' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 12, color: '#9ca3af' }}>Click any value to edit — every change is logged</span>
            </div>
            {FUNNEL_ORDER.map(funnel => {
              const frows = ueByFunnel[funnel] || []
              if (frows.length === 0) return null
              const fCollapsed = collapsedUEFunnels.has(funnel)
              return (
                <div key={funnel} style={{ marginBottom: 20 }}>
                  {funnelHeader(fCollapsed, () => toggle(collapsedUEFunnels, funnel, setCollapsedUEFunnels), funnel, frows.length) as any}
                  {!fCollapsed && (
                    <div style={{ border: '1px solid #e5e7eb', borderTop: 'none', borderRadius: '0 0 8px 8px', overflow: 'hidden', background: '#fff' }}>
                      {/* SKU header row */}
                      <div style={{ display: 'grid', gridTemplateColumns: `200px repeat(${frows.length}, 1fr)`, borderBottom: '2px solid #e5e7eb', background: '#f9fafb' }}>
                        ...
                        {frows.map(r => (
                          <div key={r.sku} style={{ padding: '10px 14px', borderLeft: '1px solid #e5e7eb' }}>
                            ...
                            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>{r.product_name?.slice(0, 30)}{r.product_name?.length > 30 ? '…' : ''}</div>
                      ...
                      {[
                        ...
                        { label: 'Country', key: 'manufacturer_country' },
                        { label: 'HTS Code', key: 'hts_code' },
                        { label: 'UPC', key: 'upc' },
                        { label: 'ASIN', key: 'asin' },
                      ].map(({ label, key }) => (
                        <div key={key} style={{ display: 'grid', gridTemplateColumns: `200px repeat(${frows.length}, 1fr)`, borderBottom: '1px solid #f3f4f6' }}>
                          <div style={{ padding: '8px 14px', fontSize: 12, color: '#6b7280', background: '#fafafa' }}>{label}</div>
                          {frows.map(r => <div key={r.sku} style={{ padding: '8px 14px', fontSize: 12, borderLeft: '1px solid #f3f4f6', color: '#374151' }}>{(r as any)[key] || '—'}</div>)}
                        </div>
                      ))}
                      {/* Cost groups */}
                      {UE_GROUPS.map(group => {
                        const groupKey = funnel + '___' + group.label
                        const gCollapsed = collapsedUEGroups.has(groupKey)
                        return (
                          <div key={group.label}>
                            <div onClick={() => toggle(collapsedUEGroups, groupKey, setCollapsedUEGroups)} style={{ display: 'grid', gridTemplateColumns: `200px 1fr`, borderBottom: '1px solid #e5e7eb', background: '#f3f4f6', cursor: 'pointer', userSelect: 'none' }}>
                              <div style={{ padding: '7px 14px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: '#374151', display: 'flex', alignItems: 'center', gap: 6 }}>
                                <span style={{ transform: gCollapsed ? 'rotate(-90deg)' : 'none', transition: 'transform 0.15s', color: '#9ca3af' }}>▾</span>
                                {group.label}
                              </div>
                              <div />
                            </div>
                            {!gCollapsed && group.fields.map(({ key, label }) => (
                              <div key={key} style={{ display: 'grid', gridTemplateColumns: `200px repeat(${frows.length}, 1fr)`, borderBottom: '1px solid #f3f4f6' }}>
                                ...
                                {frows.map(r => {
                                  ...
                                  const isDollar = key.includes('cost') || key.includes('cogs') || key.includes('fees') || key.includes('price') || key.includes('profit') || key.includes('pack') || key.includes('shipping') || key.includes('inspection') || key.includes('commission') || key.includes('tariff_cost') || key.includes('inbound') || key.includes('storage') || key.includes('referral')
                                  ...
                                  return (
                                    <div key={r.sku} onClick={() => isEditable ? (setEditingUE({ row: r, field: key, label }), setUeNewValue(val != null ? String(val) : '')) : null} style={{ padding: '8px 14px', fontSize: 12, borderLeft: '1px solid #f3f4f6', color: val == null ? '#d1d5db' : key.includes('profit') && val < 0 ? '#dc2626' : '#374151', cursor: isEditable ? 'pointer' : 'default', background: isEditable ? 'transparent' : 'transparent' }}>
        ...
        {!loading && tab === 'import' && (
          <div style={{ maxWidth: 640 }}>
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 24 }}>
              ...
              <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>Shopify Admin → Orders → Export → CSV. Duplicate orders are skipped automatically.</p>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 500, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', maxHeight: '90vh', overflow: 'auto' }}>
            ...
            {([{label:'TO Number',field:'to_number',type:'text'},{label:'Destination',field:'destination',type:'select',opts:DESTINATIONS},{label:'SKU',field:'sku',type:'sku'},{label:'Qty',field:'qty',type:'number'},{label:'Pick Up Date',field:'pick_up_date',type:'date'},{label:'ETA',field:'eta_destination',type:'date'},{label:'Shipping Method',field:'shipping_method',type:'select',opts:SHIPPING_METHODS},{label:'Status',field:'status',type:'select',opts:STATUSES_TO}] as any[]).map(({ label, field, type, opts }) => (
              <div key={field} style={{ marginBottom: 12 }}>
                ...
                {type === 'select' ? (
                  <select value={(toEdits as any)[field] ?? ''} onChange={e => setToEdits(p => ({ ...p, [field]: e.target.value }))} style={{ width: '100%', padding: '7px 10px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' }}>
      ...
      {editingUE && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 440, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setEditingUE(null)} style={{ flex: 1, padding: '9px 0', border: '1px solid #e5e7eb', borderRadius: 8, cursor: 'pointer', background: '#fff', fontSize: 14 }}>Cancel</button>
      ...
      {showTOChangelog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 600, maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>Transfer Order History</h3>
              <button onClick={() => setShowTOChangelog(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#6b7280' }}>×</button>
            </div>
            {toChangelog.length === 0 ? <div style={{ color: '#9ca3af', textAlign: 'center', padding: 40 }}>No changes recorded for this TO.</div>
            : toChangelog.map(c => (
              <div key={c.id} style={{ padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontWeight: 600, fontSize: 13 }}>{c.field_changed}</span>
                  <span style={{ fontSize: 12, color: '#9ca3af' }}>{new Date(c.changed_at).toLocaleString('en-GB')}</span>
                </div>
                <div style={{ fontSize: 13, marginBottom: 4, color: '#374151' }}>{c.previous_value || '—'} → <strong>{c.new_value}</strong></div>
                <div style={{ fontSize: 12, color: '#6b7280', fontStyle: 'italic' }}>{c.comment}</div>
              </div>
            ))}
          </div>
      ...
      {/* ── Cost Changelog Modal ── */}
      {showCostChangelog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 600, maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, margin: 0 }}>Cost Change Log — {showCostChangelog}</h3>
              <button onClick={() => setShowCostChangelog(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#6b7280' }}>×</button>
            </div>
            {costChangelog.length === 0 ? <div style={{ color: '#9ca3af', textAlign: 'center', padding: 40 }}>No cost changes recorded yet.</div>
            : costChangelog.map(c => (
              <div key={c.id} style={{ padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontWeight: 600, fontSize: 13 }}>{c.field_changed}</span>
